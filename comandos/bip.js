...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!ip) {
      message.channel.send('Debes proporcionar una IP.');
      return;
    }

    try {
      // Realizar la consulta para obtener los nombres de usuarios y sus respectivas ID filtrando por IP
      const sqlQuery = 'SELECT ID, Name FROM PlayaRP WHERE RegIP = ? OR IP = ?';
      const result = await executeQuery(sqlQuery, [ip, ip]);

      if (result.length === 0) {
        message.channel.send('No se encontraron usuarios con la IP proporcionada.');
        return;
        message.delete()
      }
      ...
      while (remainingLines.length > 0) {
        ...
        const embed = new MessageEmbed()
          .setColor('#00ff00')
          .setTitle('Usuarios Filtrados por IP')
