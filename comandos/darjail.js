...
module.exports = {
  ...
  async execute(client, message, commandArgs) {
    ...
    const jailMinutos = parseInt(args[2]);

    if (!jailMinutos || isNaN(jailMinutos)) {
      message.channel.send('No has establecido un tiempo.');
      return;
    }

    if (jailMinutos > 360) {
      message.channel.send('No puedes establecer más de 360 minutos.');
      return;
    }
    try {
      ...
      const jailSegundos = jailMinutos * 60;
      ...
      const sql = 'UPDATE PlayaRP SET Prison = ? WHERE ID = ?';
      const result = await executeQuery(sql, [jailSegundos, usuarioId]);

      if (result.affectedRows > 0) {
        // Se ha actualizado el tiempo de jail del usuario en la base de datos de PlayaRP
        const mensaje = `Se han añadido ${jailMinutos} minutos de jail a ${Usuario}.`;
        ...
        const channelId = '1244255628394889227'; // Cambia por el ID del canal destino
        const channel = client.channels.cache.get(channelId);
        if (channel && channel.isText()) {
          const embed = new MessageEmbed()
            .setTitle(`Sanción aplicada a: ${Usuario}(${usuarioId})`)
            .setDescription(`El usuario ${autor.username}#${autor.discriminator}(${autorId}) dio ${jailMinutos} minutos de jail a ${Usuario}.`)
      ...
      console.error('Error al dar el tiempo de jail al usuario: ', error);
    } finally {
